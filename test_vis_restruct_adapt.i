...
v_x_BC_top = 1e-9 # Test with higher strain rate 1e-9 is around do_ep = 1e-12
...
# [Adaptivity]
#   marker = marker # this specifies which marker from 'Markers' subsection to use
#   initial_steps = 0 # run adaptivity 2 times, recomputing solution, indicators, and markers each time
#   max_h_level = 2
...
[Materials] # For test reduce all pressure by 4 oom
  ...
  [serp]
    type = Serpentinite # lowered visc by 2 oom
    a_eta = 0.4         # factor
    atg_G = 39e9        # Pa
    atg_K = 75e9        # Pa
...
[Executioner]
  ...
  # Relax tolerances
  nl_abs_tol = 1e-6 # Accept 2e-6 as converged
  nl_rel_tol = 1e-6
  nl_max_its = 25
